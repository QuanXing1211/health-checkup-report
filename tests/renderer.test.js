...
const data = {
  ...
  riskOverview: {
    ...
    coreBusinessSystemRanking: ['OA 系统', 'MES 系统', 'ERP 系统'],
    maxRiskSystem: 'OA 系统',
    securityRiskTotal: 47,
    highAndAboveRiskCount: 20,
    incidentGptStats: {
      total: 10,
      hostCompromise: {
        total: 4
  ...
  riskDetails: {
    ...
    closedEvents: 68,
    containedEvents: 72,
    managedAssetEvents: 68,
    managedAssetContainedEvents: 27,
    processingEvents: 18,
    closeRate: 79,
    highRiskIncidentExamples: {
      ...
      c2Connections: [
        {
          ioc: '1.1.1.2、malicious.example.com',
          affectedAsset: '192.168.1.221',
          lastOccurredAt: '2025-06-19 00:36:52',
          disposalStatus: '已闭环'
        }
...
<p>风险业务数 <span data-field="riskBiz">0</span> 个；风险资产数 <span data-field="riskAssets">0</span> 个。</p>
<p>影响资产 <span data-field="impactAssets">0</span> 个。</p>
...
<p>托管资产事件 <span data-field="riskDetails.managedAssetEvents">0</span> 起；事件遏制 <span data-field="riskDetails.containedEvents">0</span> 起（其中托管资产事件遏制 <span data-field="riskDetails.managedAssetContainedEvents">0</span> 起）；</p>
...
<p>本次安全体检中，您的核心业务系统「<strong>{{ riskOverview.coreBusinessSystemRanking.0 }}</strong>、<strong>{{ riskOverview.coreBusinessSystemRanking.1 }}</strong>、<strong>{{ riskOverview.coreBusinessSystemRanking.2 }}</strong>等」存在 <strong>{{ riskOverview.securityRiskTotal }}</strong> 个安全风险，其中【<strong>{{ riskOverview.maxRiskSystem }}</strong>】风险较大，系统下的资产存在 <strong>{{ riskOverview.highAndAboveRiskCount }}</strong> 个高危及以上的安全风险。</p>
...
assert(html.includes('风险业务数 <span data-field="riskBiz">9</span> 个；风险资产数 <span data-field="riskAssets">14</span> 个。'));
assert(html.includes('影响资产 <span data-field="impactAssets">12</span> 个。'));
...
assert(html.includes('托管资产事件 <span data-field="riskDetails.managedAssetEvents">68</span> 起；事件遏制 <span data-field="riskDetails.containedEvents">72</span> 起（其中托管资产事件遏制 <span data-field="riskDetails.managedAssetContainedEvents">27</span> 起）；'));
...
assert(html.includes('本次安全体检中，您的核心业务系统「<strong>OA 系统</strong>、<strong>MES 系统</strong>、<strong>ERP 系统</strong>等」存在 <strong>47</strong> 个安全风险，其中【<strong>OA 系统</strong>】风险较大，系统下的资产存在 <strong>20</strong> 个高危及以上的安全风险。'));
...
assert(html.includes('<tbody data-repeat="riskDetails.highRiskIncidentExamples.viruses"><tr><td>192.168.1.221</td><td><span class="sr-event-name">a3f2…8c1d<br>b7e1…4f9a</span></td><td>2025-06-19 00:36:52</td><td><span class="sr-tag sr-tag--light sr-tag--info">已闭环</span></td></tr></tbody>'));
assert(html.includes('<tbody data-repeat="riskDetails.highRiskIncidentExamples.c2Connections"><tr><td>192.168.1.221</td><td><span class="sr-event-name">1.1.1.2<br>malicious.example.com</span></td><td>2025-06-19 00:36:52</td><td><span class="sr-tag sr-tag--light sr-tag--info">已闭环</span></td></tr></tbody>'));
